...
/**
 * Class ProductAttributesImporter
 * 
...
class ProductAttributesImporter extends FormBase
{
...
if ($product_variation) {
        ...
        if(isset($format) && !empty($format)){
                ...
                $product_variation->set('attribute_select_format', $format_attribute_id);
        }
        else{
                $product_variation->set('attribute_select_format',  NULL);   
        }
        ...
        if(isset($material_thickness) && !empty($material_thickness)){
                ...
                $product_variation->set('attribute_select_materials', $thickness_attribute_id);
        }
        else{
                $product_variation->set('attribute_select_materials', NULL);
        }
      ...
      if(isset($color_image_filename) && !empty($color_image_filename)){
        ...
        \Drupal::logger('color_attribute'.$key)->notice('<pre><code>'.print_r($color_attribute,TRUE).'</code></pre>');
        if (isset($color_attribute) && !empty($color_attribute)) {
                $color_image_destination = 'public://product_images/' . $color_image_filename;
                // \Drupal::logger('color_image '.$key)->notice('<pre><code>'.print_r($color_image_destination,TRUE).'</code></pre>');
                  ...
                  // Save the file.
                  $file->save();
                  // \Drupal::logger('color_image_field '.$key)->notice('<pre><code>'.print_r( $color_attribute->get('field_upload_color_palette_image'),TRUE).'</code></pre>');
